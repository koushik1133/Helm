...
-- This is a HISTORICAL snapshot (~phase 55–58). It defines PRE-HARDENING,
-- NON-org-scoped privileged SECURITY DEFINER bodies (admin_*, create_quote,
-- confirm_quote, generate_approval_token, mark_paid). Running it on a DB that
-- has the numbered phase files would REVERT tenant isolation (cross-org role
-- change / quote confirmation / payment). The canonical install is the ordered
-- supabase/phaseNN-name.sql files. See supabase/full-schema/README.md.
-- =========================================================================
-- #########################################################################
...
begin
  ...
  -- Random 6-digit code — never a hardcoded/predictable PIN (PR-AUTH-01). In
  -- simulation the code is echoed via dev_code below; when sms_live is on, MSG91
  -- generates the real code via the send-otp Edge Function.
...
-- PIN (123456). Removed — a fixed OTP let a token holder self-approve. This
-- section now installs the SAME random-code request_otp() and can never restore
-- the fixed PIN. Idempotent. Depends on: otp-payments.sql.
-- =========================================================================
create or replace function public.request_otp(p_token uuid, p_phone text)
returns jsonb language plpgsql security definer set search_path = public, extensions as $$
declare q public.quotes; code text; recent int; live boolean;
